...
module.exports.run = async(client, message, args)  => {
 ...
 if(!message.member.roles.cache.get(ayarlar.BanYetkilisi) && !message.member.hasPermission('781466109857103902')) return message.channel.send('Bu Komutu Kullanmak İçin Yetkiniz Bulunmamakta.')
...
if(member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send('Etiketlenen Kullanıcı Sizden Üst/Aynı Pozisyonda.')
  member.ban({days: 7, reason: reason})
        ...
        .setColor(`GREEN`)
        .setDescription(`<@${member.id}> (\`${member.id}\`) Adlı Kullanıcı <@${message.author.id}> Tarafından Sunucudan Yasaklandı \<a:yuklen:782271167909462037> .
