...
if(!message.member.roles.cache.has(ayarlar.JailYetkilisi) && !message.member.hasPermission('781466110574198804')) return message.channel.send('Bu Komutu Kullanmak İçin Yetkiniz Bulunmamakta.')
...
if(!sebep) return message.channel.send('Bir Sebep Belirt.')
  ...
  .setDescription(`<@${kişi.id}> (\`${kişi.id}\`) üyesi sunucuda cezalandırıldı \<a:yldz:782269240953798667> .
...
• Sebep: \`${sebep}\``)
  ...
  .setColor('#494459')
  .setAuthor(message.author.username, message.author.avatarURL({ dynamic : true }))
  .setDescription(`<@${kişi.id}> (\`${kişi.id}\`) üyesinin cezası sonlandı <@&781466110574198804> rollerini vermeyi unutma unutma \<a:yldz:782269240953798667> .
